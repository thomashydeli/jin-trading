...
def graph_close(stock, start_date, end_date):
  df = web.DataReader(stock, start = start_date, end= end_date)['Close']
  df = pd.DataFrame(df)
...
def graph_open(stock, start_date, end_date):
  df = web.DataReader(stock, start = start_date, end= end_date)['Open']
  df = pd.DataFrame(df)
  plt.figure(figsize=(20,10))
  plt.plot(df.index, df[stock])
  plt.xlabel("Date")
...
def graph_volume(stock, start_date, end_date):
  df = web.DataReader(stock, start = start_date, end= end_date)['Volume']
  df = pd.DataFrame(df)
  plt.figure(figsize=(20,10))
  plt.plot(df.index, df[stock])
  plt.xlabel("Date")
...
def graph_adj_close(stock, start_date, end_date):
  df = web.DataReader(stock, start = start_date, end= end_date)['Adj Close']
  df = pd.DataFrame(df)
  plt.figure(figsize=(20,10))
  plt.plot(df.index, df[stock])
  plt.xlabel("Date")
...
def covariance(stocks, start_date, end_date, days):
  df = web.DataReader(stocks, start = start_date, end= end_date )['Close']
  df = pd.DataFrame(df)
  returns = df.pct_change()
...
def ohlcv(stock, start_date, end_date):
  df = web.DataReader(stock, start = start_date, end= end_date )
  df = pd.DataFrame(df)
...
def efficient_frontier(stocks, start_date, end_date, iterations):
  ...
  stock_raw = web.DataReader(stocks, start=start_date, end=end_date)
